...
Preiously a program called mkint-h.c tested the current runtime environment
and created a file INT.h filled with #defines that specify the details of
the current C system. This has been renamed to mkdefines-h.c and extended.
It creates a file defines.h.
...
(c) Copyright:  Martin Richards  15 Aug 2023
...
rasterp.o and are called fastsys and rastpos. fasterp.o contains the definition
...
interpreters using the slow and fast commands.
 
The rastering version can generate memory reference data that allows
memory address/time graphs to be constructed. It should be linked as a
replacement for cintasm.  The rastering data can be converted by the
rast2ps command to pictures of the execution history of a program.
